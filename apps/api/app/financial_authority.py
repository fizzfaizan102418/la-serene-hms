    ...
    if target.transaction_type == "folio_payment" and target.folio_id is not None:
        ...
        if payment_id is not None:
            ...
            if amount is not None:
                ...
                if has_history and money(amount) > remaining: raise HTTPException(status_code=409, detail="Payment exceeds the remaining authoritative folio balance")
            ...
            if payment_id is not None:
                ...
                if has_payment_history:
                    ...
                    if payment_amount is not None:
                        ...
                        if money(refund_amount) > refundable: raise HTTPException(status_code=409, detail=f"Refund exceeds refundable payment balance of {refundable}")
    if target.transaction_type in {"deposit_applied", "deposit_refund"} and target.reference_type == "deposit" and target.reference_id:
        ...
        if deposit_id is not None:
            ...
            if stay_id is not None:
                ...
                if has_history and money(deposit_amount) > remaining: raise HTTPException(status_code=409, detail="Deposit transaction exceeds the remaining authoritative deposit balance")

@event.listens_for(FinancialTransaction, "before_insert")
def guard_concurrent_financial_posting(mapper, connection, target: FinancialTransaction) -> None: _guard_concurrent_posting(connection, target)
