...
def _has_transaction(connection, reference_type: str, reference_id: int) -> bool:
    return connection.execute(
        ...
        .where(
            ...
            FinancialTransaction.reference_id == str(reference_id),
        )
        .limit(1)
...
def derive_invoice_total_from_ledger(mapper, connection, target: Invoice) -> None:
    ...
    settlement_credits = connection.execute(
        ...
        .join(FinancialTransaction, FinancialTransaction.id == LedgerEntry.transaction_id)
        .where(
            LedgerEntry.folio_id == target.folio_id,
            LedgerEntry.account == "Guest Receivables",
            LedgerEntry.direction == "credit",
            FinancialTransaction.status == "posted",
            FinancialTransaction.transaction_type.in_(("folio_payment", "deposit_applied")),
    ...
    target.total = money(
        max(
            Decimal("0.00"),
            Decimal(debit_total) - Decimal(credit_total) + Decimal(settlement_credits) - Decimal(refund_debits),
        )
    )
