...
def _receivable_sum(connection, *, folio_id: int, direction: str) -> Decimal:
    value = connection.execute(
        ...
        .where(LedgerEntry.folio_id == folio_id, LedgerEntry.account == "Guest Receivables", LedgerEntry.direction == direction, FinancialTransaction.status == "posted")
    ).scalar_one()
...
def post_folio_charge_authoritative(
    ...
    *,
    ...
    transaction = post_transaction(
        ...
        idempotency_key=f"folio-charge:{item_id}",
        lines=[
    ...
    if discount > 0:
        post_transaction(
            ...
            description=f"Discount for folio item #{item_id}",
            reference_type="folio_item_discount",
            reference_id=str(item_id),
            folio_id=folio_id,
            reservation_id=reservation_id,
            created_by=created_by,
            idempotency_key=f"folio-discount:{item_id}",
            lines=[
    ...
    if category.strip().lower() in FOOD_CATEGORIES:
        ...
        if service_charge > 0:
            post_transaction(
                ...
                description=f"Food service charge for folio item #{item_id}",
                reference_type="folio_item_service_charge",
                reference_id=str(item_id),
                folio_id=folio_id,
                reservation_id=reservation_id,
                created_by=created_by,
                idempotency_key=f"folio-service-charge:{item_id}",
                lines=[
...
def prevent_posted_folio_item_update(mapper, connection, target: FolioItem) -> None:
    if connection.execute(select(FinancialTransaction.id).where(FinancialTransaction.reference_type == "folio_item", FinancialTransaction.reference_id == str(target.id)).limit(1)).scalar_one_or_none() is not None:
...
def prevent_payment_mutation(mapper, connection, target: Payment) -> None:
    if connection.execute(select(FinancialTransaction.id).where(FinancialTransaction.reference_type == "payment", FinancialTransaction.reference_id == str(target.id)).limit(1)).scalar_one_or_none() is not None:
...
def prevent_refund_mutation(mapper, connection, target: PaymentRefund) -> None:
    if connection.execute(select(FinancialTransaction.id).where(FinancialTransaction.reference_type == "payment_refund", FinancialTransaction.reference_id == str(target.id)).limit(1)).scalar_one_or_none() is not None:
...
def prevent_deposit_mutation(mapper, connection, target: DepositTransaction) -> None:
    if connection.execute(select(FinancialTransaction.id).where(FinancialTransaction.reference_type == "deposit", FinancialTransaction.reference_id == str(target.id)).limit(1)).scalar_one_or_none() is not None:
