...
def ensure_default_rate_segment(db: Session, stay: Stay) -> list[StayRateSegment]:
    ...
    if segments:
        return segments
    segment = StayRateSegment(stay_id=stay.id, from_date=stay.check_in, to_date=stay.check_out, rate=money(stay.agreed_rate + stay.discount_amount), discount_percent=stay.discount_percent, discount_amount=stay.discount_amount, source="migration")
...
def add_stay_occupant(stay_id: int, payload: OccupantCreate, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    ...
    if check_out <= check_in or check_in < stay.check_in or check_out > stay.check_out: raise HTTPException(status_code=400, detail="Occupant dates must be within the stay")
    if payload.is_primary:
        for occupant in db.scalars(select(StayOccupant).where(StayOccupant.stay_id == stay_id)).all(): occupant.is_primary = False
    ...
    db.add(occupant); db.flush(); audit(db, user.id, "add", "stay_occupant", occupant.id, {"stay_id": stay_id, "guest_id": payload.guest_id}); db.commit()
...
def list_deposits(stay_id: int, db: Session = Depends(get_db), _: User = Depends(require_roles("admin", "reception"))):
    ...
    for item in rows:
        balance += item.amount if item.transaction_type in {"received", "adjusted"} else -item.amount
...
def create_deposit(stay_id: int, payload: DepositCreate, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    ...
    if payload.transaction_type == "received" and stay.deposit_required > 0 and new_balance > stay.deposit_required: raise HTTPException(status_code=409, detail="Deposit received exceeds the required deposit")
    ...
    db.add(tx); db.flush(); stay.deposit_received = new_balance; audit(db, user.id, "deposit", "stay", stay_id, {"transaction_type": payload.transaction_type, "amount": str(payload.amount), "new_balance": str(new_balance)}); db.commit()
...
def move_stay_room(stay_id: int, payload: RoomMoveCreate, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    ...
    move = RoomMove(stay_id=stay.id, from_room_id=source.id, to_room_id=target.id, reason=payload.reason, created_by=user.id); db.add(move); db.flush()
    ...
    db.add(ReservationRoom(reservation_id=stay.reservation_id, room_id=target.id)); stay.room_id = target.id
    source.status = "dirty"; target.status = "occupied"; audit(db, user.id, "move", "stay", stay.id, {"from_room_id": source.id, "to_room_id": target.id, "move_id": move.id}); db.commit()
    return {"move_id": move.id, "stay_id": stay.id, "from_room_id": source.id, "to_room_id": target.id, "effective_at": move.effective_at}
...
def split_reservation(reservation_id: int, payload: ReservationSplitCreate, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    ...
    if not set(payload.room_ids).issubset(set(current_rooms)): raise HTTPException(status_code=400, detail="Split contains a room not assigned to the reservation")
    ...
    if source_folio and db.scalar(select(Payment.id).where(Payment.folio_id == source_folio.id)): raise HTTPException(status_code=409, detail="Cannot split a reservation after payments have been posted")
    ...
    db.add(new_reservation); db.flush(); new_folio = Folio(reservation_id=new_reservation.id, status="open"); db.add(new_folio); db.flush()
    ...
    for stay in selected_stays:
        stay.reservation_id = new_reservation.id; stay.guest_id = guest_id
        if source_folio:
            for item in db.scalars(select(FolioItem).where(FolioItem.stay_id == stay.id, FolioItem.folio_id == source_folio.id)).all(): item.folio_id = new_folio.id
    split = ReservationSplit(source_reservation_id=source.id, new_reservation_id=new_reservation.id, reason=payload.reason, split_check_in=source.check_in, split_check_out=source.check_out, created_by=user.id)
    db.add(split); audit(db, user.id, "split", "reservation", source.id, {"new_reservation_id": new_reservation.id, "room_ids": payload.room_ids}); db.commit()
