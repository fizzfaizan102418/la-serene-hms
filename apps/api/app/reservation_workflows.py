...
def create_reservation_workflow(payload: ReservationWorkflowCreate, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    ...
    for item, _, _, _ in stay_values:
        db.add(ReservationRoom(reservation_id=reservation.id, room_id=item.room_id))
    db.flush()

    triggered_stays = {s.room_id: s for s in db.scalars(select(Stay).where(Stay.reservation_id == reservation.id)).all()}
    if set(triggered_stays) != set(room_ids):
        raise HTTPException(status_code=500, detail="Room stay allocation could not be initialized")
    ...
    for item, discount, net, occupant_id in stay_values:
        stay = triggered_stays[item.room_id]
        stay.guest_id = occupant_id
        stay.status = "reserved"
        stay.check_in = payload.check_in
        stay.check_out = payload.check_out
        stay.agreed_rate = net
        stay.discount_percent = money(item.discount_percent)
        stay.discount_amount = discount
        stay.payment_due_policy = payload.payment_policy
        stay.deposit_required = money(net * stay_days)
        stay.deposit_received = min(per_room_deposit, stay.deposit_required)
        stay.notes = payload.notes
...
def update_reservation(reservation_id: int, payload: ReservationUpdate, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    ...
    for stay in db.scalars(select(Stay).where(Stay.reservation_id == reservation_id)).all(): stay.check_in = check_in; stay.check_out = check_out
