...
class ReservationRoomSetup(BaseModel):
    ...
    fixed_discount: float = Field(default=0, ge=0)
...
class ReservationWorkflowCreate(BaseModel):
    ...
    deposit_received: float = Field(default=0, ge=0)
...
def create_reservation_workflow(payload: ReservationWorkflowCreate, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    ...
    if payload.deposit_received > sum(max(0.0, item.agreed_rate - min(item.agreed_rate, item.fixed_discount + (item.agreed_rate * item.discount_percent / 100))) for item in payload.rooms) * max(1, (payload.check_out - payload.check_in).days):
    ...
    db.add(folio)
    db.flush()
    per_room_deposit = (payload.deposit_received / len(room_ids)) if room_ids else 0
    for item in payload.rooms:
        db.add(ReservationRoom(reservation_id=reservation.id, room_id=item.room_id))
        gross = round(item.agreed_rate, 2)
        pct = round(gross * item.discount_percent / 100, 2)
        discount = min(gross, pct if item.discount_percent else round(item.fixed_discount, 2))
        ...
        stay = Stay(reservation_id=reservation.id, room_id=item.room_id, guest_id=occupant_id, status="reserved", check_in=payload.check_in, check_out=payload.check_out, agreed_rate=net, discount_percent=item.discount_percent, discount_amount=discount, payment_due_policy=payload.payment_policy, deposit_required=net * max(1, (payload.check_out - payload.check_in).days), deposit_received=per_room_deposit, notes=payload.notes)
        db.add(stay)
    ...
    audit(db, user.id, "create_workflow", reservation.id, {"room_ids": room_ids, "group_id": payload.group_id, "payment_policy": payload.payment_policy, "deposit_received": payload.deposit_received})
    db.commit()
...
def extend_stay(reservation_id: int, payload: ExtendStay, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    ...
    for stay in db.scalars(select(Stay).where(Stay.reservation_id == reservation_id)).all(): stay.check_out = payload.new_check_out
