...
def close_day(payload: ClosingConfirm | None = None, db: Session = Depends(get_db), user: User = Depends(require_roles("admin"))):
    ...
    summary = build_summary(db, today)
    audit(db, user.id, "daily_close", {"business_date": str(today), "summary": summary, "notes": payload.notes if payload else None})
    db.commit()
    return {"status": "closed", "business_date": today, "summary": summary}
