...
def post_transaction(db: Session, *, transaction_type: str, description: str, lines: list[LedgerLine | dict], created_by: int | None = None, business_date: date | None = None, reference_type: str | None = None, reference_id: str | None = None, folio_id: int | None = None, reservation_id: int | None = None, reversal_of_id: int | None = None) -> FinancialTransaction:
    if len(lines) < 2:
...
def post_folio_payment(db: Session, *, folio_id: int, reservation_id: int, payment_id: int, amount: Decimal, method: str, created_by: int) -> FinancialTransaction:
    ...
    return post_transaction(db, transaction_type="folio_payment", description=f"Folio payment #{payment_id} ({method})", reference_type="payment", reference_id=str(payment_id), folio_id=folio_id, reservation_id=reservation_id, created_by=created_by, lines=[{"account": account, "direction": "debit", "amount": amount, "folio_id": folio_id, "payment_method": method}, {"account": "Guest Receivables", "direction": "credit", "amount": amount, "folio_id": folio_id, "payment_method": method}])
...
def post_deposit_received(db: Session, *, stay_id: int, folio_id: int | None, reservation_id: int, deposit_id: int, amount: Decimal, method: str | None, created_by: int) -> FinancialTransaction:
    ...
    return post_transaction(db, transaction_type="deposit_received", description=f"Deposit received #{deposit_id}", reference_type="deposit", reference_id=str(deposit_id), folio_id=folio_id, reservation_id=reservation_id, created_by=created_by, lines=[{"account": cash_account, "direction": "debit", "amount": amount, "folio_id": folio_id, "stay_id": stay_id, "payment_method": method}, {"account": "Guest Deposits", "direction": "credit", "amount": amount, "folio_id": folio_id, "stay_id": stay_id, "payment_method": method}])
