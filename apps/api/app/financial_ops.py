...
def refund_payment(folio_id: int, payload: RefundCreate, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    folio = db.get(Folio, folio_id); payment = db.get(Payment, payload.payment_id)
    ...
    if payload.amount > refundable: raise HTTPException(status_code=409, detail=f"Refund exceeds refundable payment balance of {refundable}")
...
def ledger_reconciliation(business_date: date | None = None, db: Session = Depends(get_db), _: User = Depends(require_roles("admin", "reception"))):
    ...
    day_start, day_end = business_day_bounds(target_date)
    daily_items = db.scalars(select(FolioItem).where(FolioItem.created_at >= day_start, FolioItem.created_at < day_end)).all()
    operational_charges = money(sum((item_total(i) for i in daily_items), Decimal("0.00")) + food_service_charge(daily_items))
    ...
    daily_payments = money(db.scalar(select(func.coalesce(func.sum(Payment.amount), 0)).where(Payment.created_at >= day_start, Payment.created_at < day_end)) or 0)
    daily_refunds = money(db.scalar(select(func.coalesce(func.sum(PaymentRefund.amount), 0)).where(PaymentRefund.created_at >= day_start, PaymentRefund.created_at < day_end)) or 0)
    cash_debits = money(sum((e.amount for e in entry_rows if e.direction == "debit" and e.account in CASH_ACCOUNTS), Decimal("0.00")))
    cash_credits = money(sum((e.amount for e in entry_rows if e.direction == "credit" and e.account in CASH_ACCOUNTS), Decimal("0.00")))
    net_cash_ledger = money(cash_debits - cash_credits); net_cash_operational = money(daily_payments - daily_refunds)
    charge_difference = money(ledger_revenue - operational_charges); cash_difference = money(net_cash_ledger - net_cash_operational)
    status = "balanced" if debits == credits and charge_difference == Decimal("0.00") and cash_difference == Decimal("0.00") else "review"
    return {"business_date": target_date, "ledger": {"transactions": len(transactions), "debits": debits, "credits": credits, "balanced": debits == credits, "revenue_credits": ledger_revenue, "cash_debits": cash_debits, "cash_credits": cash_credits, "net_cash": net_cash_ledger}, "operational": {"folio_charges": operational_charges, "payments": daily_payments, "refunds": daily_refunds, "net_cash": net_cash_operational}, "reconciliation": {"charge_difference": charge_difference, "cash_difference": cash_difference, "status": status}}
...
def create_deposit_with_ledger(stay_id: int, payload: dict, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    ...
    for item in db.scalars(select(DepositTransaction).where(DepositTransaction.stay_id == stay_id)).all(): current += item.amount if item.transaction_type in {"received", "adjusted"} else -item.amount
    signed = amount if transaction_type in {"received", "adjusted"} else -amount; new_balance = money(current + signed)
    ...
    db.add(tx); db.flush()
    method = payload.get("payment_method") or "other"; cash_account = {"cash": "Cash", "card": "Card Clearing", "bank_transfer": "Bank", "other": "Other Payment"}.get(method, "Other Payment")
    if transaction_type == "received":
        post_deposit_received(db, stay_id=stay.id, folio_id=folio.id if folio else None, reservation_id=stay.reservation_id, deposit_id=tx.id, amount=amount, method=payload.get("payment_method"), created_by=user.id)
    elif transaction_type == "refunded":
        post_transaction(db, transaction_type="deposit_refund", description=f"Deposit refund #{tx.id}", reference_type="deposit", reference_id=str(tx.id), folio_id=folio.id if folio else None, reservation_id=stay.reservation_id, created_by=user.id, lines=[{"account": "Guest Deposits", "direction": "debit", "amount": amount, "stay_id": stay.id}, {"account": cash_account, "direction": "credit", "amount": amount, "stay_id": stay.id, "payment_method": payload.get("payment_method")}])
    elif transaction_type == "applied":
        post_transaction(db, transaction_type="deposit_applied", description=f"Deposit applied #{tx.id}", reference_type="deposit", reference_id=str(tx.id), folio_id=folio.id if folio else None, reservation_id=stay.reservation_id, created_by=user.id, lines=[{"account": "Guest Deposits", "direction": "debit", "amount": amount, "stay_id": stay.id}, {"account": "Guest Receivables", "direction": "credit", "amount": amount, "folio_id": folio.id if folio else None, "stay_id": stay.id}])
    else:
        post_transaction(db, transaction_type="deposit_adjustment", description=f"Deposit adjustment #{tx.id}", reference_type="deposit", reference_id=str(tx.id), folio_id=folio.id if folio else None, reservation_id=stay.reservation_id, created_by=user.id, lines=[{"account": "Guest Deposits", "direction": "credit", "amount": amount, "stay_id": stay.id}, {"account": "Deposit Adjustments", "direction": "debit", "amount": amount, "stay_id": stay.id}])
    stay.deposit_received = new_balance; audit(db, user.id, "deposit", "stay", stay_id, {"deposit_transaction_id": tx.id, "transaction_type": transaction_type, "amount": str(amount), "new_balance": str(new_balance)}); db.commit(); db.refresh(tx)
