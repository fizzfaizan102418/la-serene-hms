...
def folio_balance(db: Session, folio: Folio) -> tuple[Decimal, Decimal, Decimal]:
    ...
    if summary.total != Decimal("0.00") or summary.paid != Decimal("0.00") or db.scalar(select(FinancialTransaction.id).where(FinancialTransaction.folio_id == folio.id, FinancialTransaction.status == "posted")) is not None:
...
def refund_payment(folio_id: int, payload: RefundCreate, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"), db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    folio = db.get(Folio, folio_id); payment = db.get(Payment, payload.payment_id); key = (idempotency_key or "").strip()
    if not key: raise HTTPException(status_code=400, detail="Idempotency-Key header is required for refunds")
    if len(key) > 100: raise HTTPException(status_code=400, detail="Idempotency-Key must be 100 characters or fewer")
    if not folio or not payment or payment.folio_id != folio_id: raise HTTPException(status_code=404, detail="Folio or payment not found")
    existing_tx = db.scalar(select(FinancialTransaction).where(FinancialTransaction.idempotency_key == key))
    if existing_tx is not None:
        if existing_tx.transaction_type != "payment_refund" or existing_tx.folio_id != folio_id: raise HTTPException(status_code=409, detail="Idempotency key is already bound to another financial operation")
        ...
        if refund is None or refund.payment_id != payment.id: raise HTTPException(status_code=409, detail="Idempotent refund record is missing or incompatible")
        if refund.amount != money(payload.amount) or (refund.method or payment.method) != (payload.method or payment.method) or refund.reason != payload.reason: raise HTTPException(status_code=409, detail="Idempotency key is already bound to different refund parameters")
    ...
    if tx.reference_id != str(refund.id):
        db.delete(refund); db.flush(); replay_refund = db.get(PaymentRefund, int(tx.reference_id)) if tx.reference_id and tx.reference_id.isdigit() else None
        if replay_refund is None or replay_refund.payment_id != payment.id: db.rollback(); raise HTTPException(status_code=409, detail="Idempotent refund record is missing or incompatible")
        return _refund_response(db, folio, replay_refund, True)
    audit(db, user.id, "refund", "payment", payment.id, {"folio_id": folio_id, "refund_id": refund.id, "amount": str(amount), "method": method, "reason": payload.reason}); db.commit(); db.refresh(refund)
...
def create_adjustment(payload: AdjustmentCreate, db: Session = Depends(get_db), user: User = Depends(require_roles("admin"))):
    try:
        tx = post_transaction(db, transaction_type="adjustment", description=payload.description, reference_type="manual_adjustment", reference_id=payload.description[:50], folio_id=payload.folio_id, reservation_id=payload.reservation_id, created_by=user.id, lines=payload.lines); audit(db, user.id, "adjustment", "financial_transaction", tx.id, {"description": payload.description, "folio_id": payload.folio_id, "reservation_id": payload.reservation_id}); db.commit(); db.refresh(tx); return {"id": tx.id, "transaction_no": tx.transaction_no, "business_date": tx.business_date, "status": tx.status}
...
def create_folio_window(folio_id: int, payload: WindowCreate, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    ...
    window = FolioWindow(folio_id=folio_id, name=payload.name, payer_type=payload.payer_type, guest_id=payload.guest_id, group_id=payload.group_id); db.add(window); db.flush(); audit(db, user.id, "create", "folio_window", window.id, {"folio_id": folio_id, "name": window.name}); db.commit(); db.refresh(window)
...
def transfer_item_to_window(folio_id: int, window_id: int, payload: WindowTransfer, db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    ...
    audit(db, user.id, "route", "folio_item", item.id, {"folio_id": folio_id, "window_id": window.id}); db.commit(); return {"item_id": item.id, "folio_id": folio_id, "window_id": window.id}
...
def night_audit_reconciliation(db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    state = db.get(BusinessDateState, 1); target_date = state.current_business_date if state else date.today(); return ledger_reconciliation(target_date, db, user)
...
def create_deposit_with_ledger(stay_id: int, payload: dict, idempotency_key: str | None = Header(default=None, alias="Idempotency-Key"), db: Session = Depends(get_db), user: User = Depends(require_roles("admin", "reception"))):
    stay = db.get(Stay, stay_id)
    if not stay: raise HTTPException(status_code=404, detail="Stay not found")
    transaction_type = payload.get("transaction_type"); amount = money(payload.get("amount", 0))
    if transaction_type not in {"received", "applied", "refunded", "adjusted"} or amount <= 0: raise HTTPException(status_code=400, detail="Invalid deposit transaction")
    key = (idempotency_key or "").strip()
    if len(key) > 100: raise HTTPException(status_code=400, detail="Idempotency-Key must be 100 characters or fewer")
    if transaction_type in {"applied", "refunded"} and not key: raise HTTPException(status_code=400, detail="Idempotency-Key header is required for deposit application/refund")
    if key:
        existing_tx = db.scalar(select(FinancialTransaction).where(FinancialTransaction.idempotency_key == key))
        if existing_tx is not None:
            if existing_tx.transaction_type != f"deposit_{transaction_type}" or existing_tx.reservation_id != stay.reservation_id: raise HTTPException(status_code=409, detail="Idempotency key is already bound to another financial operation")
            deposit_id = int(existing_tx.reference_id) if existing_tx.reference_id and existing_tx.reference_id.isdigit() else None; existing_deposit = db.get(DepositTransaction, deposit_id) if deposit_id else None
            if existing_deposit is None or existing_deposit.stay_id != stay.id: raise HTTPException(status_code=409, detail="Idempotent deposit transaction is missing or incompatible")
            if money(existing_deposit.amount) != amount or existing_deposit.transaction_type != transaction_type: raise HTTPException(status_code=409, detail="Idempotency key is already bound to different deposit parameters")
    ...
    required_deposit = money(stay.agreed_rate * max(0, (stay.check_out - stay.check_in).days))
    if required_deposit > 0 and new_balance > required_deposit:
        raise HTTPException(status_code=409, detail=f"Deposit received exceeds the stay's current required deposit of {required_deposit}")
    if transaction_type == "received" and stay.deposit_required != required_deposit:
        stay.deposit_required = required_deposit
    ...
    try:
        ...
        else: tx = post_transaction(db, transaction_type="deposit_adjustment", description=f"Deposit adjustment #{deposit.id}", reference_type="deposit", reference_id=str(deposit.id), folio_id=folio.id if folio else None, reservation_id=stay.reservation_id, created_by=user.id, idempotency_key=financial_key, lines=[{"account": "Guest Deposits", "direction": "credit", "amount": amount, "stay_id": stay.id}, {"account": "Deposit Adjustments", "direction": "debit", "amount": amount, "stay_id": stay.id}])
    except (ValueError, IntegrityError) as exc:
        db.rollback(); raise HTTPException(status_code=409, detail=str(exc)) from exc
    if tx.reference_id != str(deposit.id):
        db.delete(deposit); db.flush(); replay_deposit_id = int(tx.reference_id) if tx.reference_id and tx.reference_id.isdigit() else None; replay_deposit = db.get(DepositTransaction, replay_deposit_id) if replay_deposit_id else None
        if replay_deposit is None or replay_deposit.stay_id != stay.id: db.rollback(); raise HTTPException(status_code=409, detail="Idempotent deposit transaction is missing or incompatible")
    ...
    stay.deposit_received = stay_deposit_ledger_balance(db, stay.id); audit(db, user.id, "deposit", "stay", stay_id, {"deposit_transaction_id": deposit.id, "transaction_type": transaction_type, "amount": str(amount), "new_balance": str(stay.deposit_received)}); db.commit(); db.refresh(deposit)
