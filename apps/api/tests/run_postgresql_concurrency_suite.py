...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run PostgreSQL concurrency tests in an isolated database.")
...
def main() -> int:
    ...
    if not source_db or not source_user:
        print("ERROR: HMS_DATABASE_URL must include a database name and username.")
        return 2
    ...
    test_db = f"la_serene_hms_pg_concurrency_{uuid.uuid4().hex[:10]}"
    if test_db == source_db:
        print("ERROR: Refusing to use the source database as the concurrency test database.")
        return 2
    ...
    try:
        try:
            ...
            print("ERROR: Could not create the disposable PostgreSQL concurrency test database.")
        ...
        created = True
        print(f"Created isolated PostgreSQL concurrency test database: {test_db}")
        ...
        run(
            [sys.executable, "-m", "unittest", "tests.test_postgres_b2_concurrency", "-v"],
            cwd=API_ROOT,
            env={**env, "HMS_TEST_DATABASE_URL": env["HMS_DATABASE_URL"]},
        )
        print("PostgreSQL concurrency suite: PASS")
        return 0
    finally:
        if created:
            try:
                ...
                print(f"Removed isolated PostgreSQL concurrency test database: {test_db}")
