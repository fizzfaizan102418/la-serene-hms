...
<footer class="footer"><span>La Serene Hotel · Official daily closing record</span><span>${summary.posting_open ? 'Posting period open' : 'Posting period closed'}</span></footer>
  ...
  return <section className="page">
    ...
    {!archive && <section className="panel" style={{ marginBottom: 16 }}><div className="panel-head"><div><p className="muted">Current business-date activity</p><h2>What is already posted</h2></div><span>{summary.posting_open ? 'Live' : 'Closed'}</span></div><p className="muted">Room charges for the night are posted automatically during Daily Closing. Staff do not enter these charges manually.</p><div className="report-list"><div><span>Room revenue already posted today</span><strong>{money(summary.revenue.room)}</strong></div><div><span>Guest money received today</span><strong>{money(summary.payments.total + Number(summary.guest_deposits?.received_today || 0))}</strong></div><div><span>Open folio balance across active folios</span><strong>{money(summary.outstanding)}</strong></div><div><span>Active in-house reservations</span><strong>{summary.occupancy.in_house_reservations}</strong></div><div><span>Current ledger transactions</span><strong>{f.ledger_transactions}</strong></div><div><span>Current ledger status</span><strong>{f.status === 'balanced' ? 'Balanced' : 'Review required'}</strong></div></div></section>}
