...
<footer class="footer"><span>Thank you for staying with La Serene Hotel.</span><span>Official guest folio · Keep for your records</span></footer>
  ...
  return <section className="page">
    <div className="page-heading"><div><p className="muted">Manage guest payments and balances</p><h2>Billing</h2></div><span className="room-count">{registerStats.total} folios</span></div>
    ...
    <div className="billing-layout billing-workspace">
      ...
      <div className="side-stack">
        {folio && <div className="panel billing-detail-panel">
          <div className="panel-head"><div><p className="muted">Folio #{folio.id} · Reservation #{folio.reservation_id}</p><h2>Guest billing</h2></div><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}><span style={badgeStyle(selectedFolioStatus)}>{normalizeStatus(folio.status)}</span><span style={badgeStyle(selectedPaymentStatus)}>{paymentLabel(selectedPaymentStatus)}</span></div></div>
          <div className="billing-balance-banner"><div><span>Amount still due</span><strong>PKR {money(folio.balance)}</strong></div><small>{folio.balance > 0.005 ? 'This is the amount still to collect.' : 'Nothing remains to collect.'}</small></div><div className="billing-totals"><div><span>Current charges</span><b>PKR {money(folio.total)}</b></div><div><span>Payments received</span><b>PKR {money(folio.paid)}</b></div><div><span>Advance held for stay</span><b>PKR {money(folio.deposit_balance)}</b></div><div className="balance"><span>Amount still due</span><b>PKR {money(folio.balance)}</b></div>{Number(folio.deposit_balance || 0) > 0.005 && <p className="billing-help-text">An advance is being held for this stay and will not be collected again when eligible room charges are posted.</p>}</div>

          <div className="panel-head billing-section-head" style={{ marginTop: 18 }}><div><h3>Charges</h3><span className="muted">Charges already posted to this folio</span></div><span>{folio.items.length}</span></div>
          ...
          <div className="panel-head billing-section-head" style={{ marginTop: 18 }}><div><h3>Payments</h3><span className="muted">Recorded folio payments</span></div><span>{folio.payments.length}</span></div>
          <div className="folio-items">{folio.payments.length ? folio.payments.map(payment => <article key={payment.id}><div><strong>PKR {money(payment.amount)}</strong><span>{methodLabel(payment.method)}{payment.reference ? ` · ${payment.reference}` : ''}</span></div><span className="muted">Payment #{payment.id}</span></article>) : <p className="muted">No separate payment entries. See the payment and advance summary above.</p>}</div>

          {canOperate && <div className="billing-actions">{folio.status === 'open' && <button className="secondary-button" onClick={() => void addRoomCharges()}>Post room charge</button>}<button className="secondary-button" onClick={() => void printReceipt()}>Print receipt</button>{folio.status === 'open' && folio.balance === 0 && <button className="primary-button" onClick={() => void closeFolio()}>Close folio</button>}</div>}
        </div>}
        ...
        {canOperate && folio?.status === 'open' && <form className="panel form-panel" onSubmit={addPayment}>
          ...
          <div className="billing-simple-summary">
            <div><span>Current stay charges</span><strong>PKR {money(folio.total)}</strong></div>
