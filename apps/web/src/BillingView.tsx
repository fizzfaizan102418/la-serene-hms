...
export default function BillingView({ userRole, summaries, onRefresh, api }: Props) {
  ...
  function startEdit(item: Item) {
    ...
    setMessage('Editing this open folio line.');
  }

  function clearItemForm() {
    setEditingItem(null); setDescription(''); setCategory('service'); setQuantity('1'); setUnitPrice(''); setDiscount('0');
  }
  ...
  async function saveItem(event: React.FormEvent) {
    ...
    try {
      ...
      const body = { description, category, quantity: Number(quantity), unit_price: Number(unitPrice), discount: Number(discount || 0) };
      if (editingItem) {
        await api<Item>(`/api/folios/${id}/items/${editingItem.id}`, { method: 'PATCH', body: JSON.stringify(body) });
        setMessage('Charge updated.');
      } else {
        await api<Item>(`/api/folios/${id}/items`, { method: 'POST', body: JSON.stringify(body) });
        setMessage('Charge added.');
      }
      setFolio(await api<Folio>(`/api/folios/${id}`));
      clearItemForm();
      await onRefresh();
    } catch (err) { setMessage(err instanceof Error ? err.message : 'Unable to save charge'); }
  }
  ...
  async function removeItem(itemId: number) {
    ...
    try {
      const id = await ensureSelected();
      await api(`/api/folios/${id}/items/${itemId}`, { method: 'DELETE' });
      setFolio(await api<Folio>(`/api/folios/${id}`));
      if (editingItem?.id === itemId) clearItemForm();
