    ...
    <div className="billing-layout billing-workspace">
      ...
      <div className="side-stack">
        {folio && <div className="panel billing-detail-panel">
          ...
          <div className="billing-balance-banner"><div><span>Amount still due</span><strong>PKR {money(folio.balance)}</strong></div><small>{folio.balance > 0.005 ? 'This is the amount still to collect. Any advance already received is held separately for the stay.' : 'Nothing remains to collect.'}</small></div><div className="billing-totals"><div><span>Current charges</span><b>PKR {money(folio.total)}</b></div><div><span>Payments received</span><b>PKR {money(folio.paid)}</b></div><div><span>Advance held for stay</span><b>PKR {money(folio.deposit_balance)}</b></div><div className="balance"><span>Amount still due</span><b>PKR {money(folio.balance)}</b></div><p className="billing-help-text">The advance is already received and is kept for this stay. It is not collected again when the next room charge is posted.</p></div>
