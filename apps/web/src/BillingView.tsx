    ...
    <div className="billing-layout billing-workspace">
      ...
      <div className="side-stack">
        ...
        {canOperate && folio?.status === 'open' && <form className="panel form-panel" onSubmit={addDeposit}>
          <div className="panel-head"><div><p className="muted">Front desk money receipt</p><h2>Receive advance payment</h2></div><span>Money for the guest's stay</span></div>
          <p className="muted">This money is held for the guest's stay. When a room charge is posted, the available advance is applied automatically. You do not collect it again.</p>
          ...
          <div className="billing-stay-funding"><div><span>Advance held</span><strong>PKR {money(folio?.deposit_balance || 0)}</strong></div><div><span>Advance target</span><strong>PKR {money(folio?.deposit_required || 0)}</strong></div><div><span>Current folio balance</span><strong>PKR {money(folio?.balance || 0)}</strong></div></div>
        ...
        {canOperate && folio?.status === 'open' && <form className="panel form-panel" onSubmit={addPayment}>
          <div className="panel-head"><div><p className="muted">Front desk money receipt</p><h2>Receive balance payment</h2></div><span>Pay the current folio balance</span></div>
          ...
          <button className="primary-button" disabled={!selected || !amount}>Receive balance payment</button>
        </form>}
      </div>
    </div>
  </section>;
