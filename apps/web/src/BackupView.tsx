...
export default function BackupView({ api }: { api: BackupApi }) {
  ...
  async function createBackup() {
    ...
    try { const result = await api<BackupInfo>('/api/backup', { method: 'POST' }); setMessage(`Backup created: ${result.filename}`); await refresh(); }
    catch (err) { setError(err instanceof Error ? err.message : 'Unable to create backup'); setLoading(false); }
  }
  ...
  async function restore() {
    ...
    try {
      const token = localStorage.getItem('la_serene_access_token'); const body = new FormData(); body.append('file', restoreFile);
  ...
  return <section className="page">
    ...
    <section className="backup-status-strip">
      <div><span className="backup-status-dot" /> <strong>{backups.length ? 'Backup protection available' : 'No backup created yet'}</strong><small>{database || 'Database'} · {backups.length ? `${backups.length} backup record${backups.length === 1 ? '' : 's'} available` : 'Create the first safety copy before operational changes.'}</small></div>
      {backups[0] && <div className="backup-last"><span>Latest backup</span><strong>{new Date(backups[0].created_at).toLocaleString()}</strong></div>}
    </section>
