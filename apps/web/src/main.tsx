...
function DashboardView({ dashboard, management, finance, history, rooms, roomTypes, selectedDate, historicalPack }: { dashboard: Dashboard | null; management: ManagementReport | null; finance: DashboardFinance | null; history: DashboardHistoryDay[]; rooms: Room[]; roomTypes: RoomType[]; selectedDate: string | null; historicalPack: DashboardClosingPack | null }) {
  ...
  return <>
    ...
    <section className="dashboard-chart-grid">
      <section className="panel dashboard-line-panel"><div className="panel-head"><div><p className="muted">Actual hotel history</p><h2>5-Day Revenue Trend</h2></div><span>Tap a point for daily details</span></div>
        {history.length ? <div className="dashboard-line-chart"><svg viewBox="0 0 100 100" role="img" aria-label="Five-day total revenue trend"><line x1="10" y1="20" x2="90" y2="20" className="chart-grid-line" /><line x1="10" y1="56" x2="90" y2="56" className="chart-grid-line" /><line x1="10" y1="92" x2="90" y2="92" className="chart-axis-line" /><path d={revenuePath} className="chart-line revenue-line" />{revenuePoints.map(point => <circle key={`rev-${point.date}`} cx={point.x} cy={point.y} r={point.date === selectedDate ? 2.6 : 2.1} className={point.date === selectedDate ? 'chart-point revenue-point selected' : 'chart-point revenue-point'}><title>{`${formatDashboardDate(point.date)} · Revenue ${point.total_revenue.toFixed(2)} · Occupancy ${point.occupancy.toFixed(1)}% · ${point.closed ? 'Closed' : 'Open'}`}</title></circle>)}</svg><div className="line-chart-labels">{history.map(day => <span key={day.date} className={day.date === selectedDate ? 'selected' : ''}>{formatDashboardDate(day.date)}<small>PKR {Number(day.total_revenue).toLocaleString('en-PK', { maximumFractionDigits: 0 })}</small></span>)}</div><div className="dashboard-chart-detail">{selectedHistory ? <><div><span>{formatDashboardDate(selectedHistory.date)}</span><strong>PKR {Number(selectedHistory.total_revenue).toLocaleString('en-PK', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</strong></div><div><span>Occupancy</span><strong>{selectedHistory.occupancy.toFixed(1)}%</strong></div><div><span>Status</span><strong>{selectedHistory.closed ? 'Closed' : 'Open'}</strong></div></> : <p className="muted">Select a business date to inspect its details.</p>}</div> : <p className="muted">Select a business date to inspect its details.</p>}</div></div> : <p className="muted">Historical dashboard data is loading…</p>}
        <div className="chart-legend"><span><i className="legend-dot revenue" />Total revenue</span><span className="chart-legend-note">Occupancy is shown in the selected-day detail.</span></div><p className="dashboard-source">The chart uses actual reporting data. Closed dates use archived Night Audit figures; the current open date remains live.</p>
      </section>
      <section className="panel dashboard-pie-panel"><div className="panel-head"><div><p className="muted">{historical ? 'Archived cashier activity' : 'Current cashier activity'}</p><h2>Payment mix</h2></div><span>{historical ? formatDashboardDate(selectedDate!) : 'Business date'}</span></div>
        ...
        {!historical && <div className="dashboard-cashier-summary">
          <div className="cashier-summary-head"><strong>Today's balance</strong><span>Posted activity</span></div>
          <div className="cashier-summary-grid">
            <div><span>Received</span><strong>{finance ? Number(finance.payments_received).toFixed(2) : '—'}</strong></div>
            <div><span>Refunded</span><strong>{finance ? Number(finance.payments_refunded).toFixed(2) : '—'}</strong></div>
            <div><span>Net posted</span><strong>{finance ? Number(finance.payments_net).toFixed(2) : '—'}</strong></div>
            <div><span>Guest outstanding</span><strong>{management ? Number(management.receivables.outstanding).toFixed(2) : '—'}</strong></div>
          </div>
          <p className="dashboard-source">Net posted = received less refunds. Guest outstanding is receivable balance, not cash held on hand.</p>
        </div>}
      </section>
    </section>
